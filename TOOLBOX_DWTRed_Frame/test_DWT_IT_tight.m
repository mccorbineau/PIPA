...
%transform�e en ondelettes invariance par translation
wc = FWT2_TI_tight(x,J-jm,qmf);

%reconstruction
y = IWT2_TI_tight(wc,J-jm,qmf);
...
%au d�part sont sous la forme [[vert horz diag]' vert horz diag app]'
ind=1;
for j=1:jm,
    for ml=1:Ml,
        for mc=1:Mc,
...
%Est-ce que la norme est conserv�e ?
%Norme des coefs r�arrang�s
Nwc = 0;
for r = 1:jm
    for m1 = 1:Ml
        for m2 = 1:Mc
...
%diff�rence avec la norme des coefs de depart
...
%Ici wr et wc doivent �tre exactement les m�mes
if norm(wc-wr)~=0
...
tiu=IWT2_TI_adj_tight(u,J-jm,qmf);
%il faut v de la taille image
v=randn(size(x));
tv=FWT2_TI_tight(v,J-jm,qmf);

disp('Premier prod scalaire : ')
(u(:)'*tv(:))
disp('Deuxi�me prod scalaire : ')
(tiu(:)'*v(:))